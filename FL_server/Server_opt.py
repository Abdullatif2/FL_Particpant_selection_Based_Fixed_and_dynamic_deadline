...
from ..Server_Base import BaseFedarated
...
class Server(BaseFedarated):
    ...
    def train(self):
        ...
        for i in range(self.num_rounds):
            ...
            indices, selected_clients,Datasize = self.select_clients(i, model_size, Flops, num_clients=self.clients_per_round)  # uniform sampling
        ...
        with open('./results/proposedtestaccu'+'{}_{}.csv'.format(self.parameters['dataset'],  self.num_rounds), 'a+', newline='') as resultsfile:
            wr = csv.writer(resultsfile)
            wr.writerow(self.rs_glob_acc)
        with open('./results/proposedtrainaccu'+'{}_{}.csv'.format(self.parameters['dataset'], self.num_rounds), 'a+', newline='') as resultsfile1:
            wr = csv.writer(resultsfile1)
            wr.writerow(self.rs_train_acc)  
        with open('./results/proposedtestloss'+'{}_{}.csv'.format(self.parameters['dataset'], self.num_rounds), 'a+', newline='') as resultsfile2:
            wr = csv.writer(resultsfile2)
            wr.writerow(self.rs_train_loss) 
        with open('./results/lower_upper_bound'+'{}_{}.csv'.format(self.parameters['dataset'],  self.num_rounds), 'a+', newline='') as resultsfile:
            wr = csv.writer(resultsfile)
            wr.writerow(self.lowerbound)  
            wr.writerow(self.upperbound) 
